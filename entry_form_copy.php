...
<div class="row">
	<div id="page-title" class="col-xs-8 left">
		<h1><span class="glyphicon glyphicon-check"></span>  Picks</h1>
	</div>
	<div id="page-title" class="col-xs-4 right">
		<?php
		//display week nav
		$sql = "select distinct weekNum from " . DB_PREFIX . "schedule order by weekNum;";
		$query = $mysqli->query($sql);
		$weekNav = '<div class="btn-group"><button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Week ' . $week . ' <span class="caret"></span></button>';
		$weekNav .= '	<ul class="dropdown-menu"> ';
		$i = 0;
		if ($query->num_rows > 0) {
			while ($row = $query->fetch_assoc()) {
				if ($week !== (int)$row['weekNum']) {
					$weekNav .= '<li><a href="entry_form.php?week=' . $row['weekNum'] . '">Week ' . $row['weekNum'] . '</a></li>';
				} else {
					$weekNav .= $row['weekNum'];
				}
				$i++;
